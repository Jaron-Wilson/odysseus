...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_devices": "List, register or remove the user's devices (phone, tablet, desktop) and see what each one can be told to do. Use for 'what devices do I have', 'register my phone', 'add a device', or before sending a command so you know the device supports it.",
    "notify_device": "Send a notification or a command to one of the user's own devices (phone, tablet, desktop) over ntfy. Use for 'send it to my phone', 'notify me', 'text me when it is done', 'remind me on my phone', 'push this to my phone', or to tell a device to do something it already knows how to do, such as launching an app. Pass device to pick which one; omit it for the default.",
