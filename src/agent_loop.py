...
```
Hand a CODING task to the Claude Code CLI on this host: it reads the project, edits files and runs commands itself, and its console streams back live. Use it for work too large or intricate for single tool calls here — a multi-file refactor, a bug hunt across a codebase, a build that has to be run and iterated on — or when the user asks for it by name.
